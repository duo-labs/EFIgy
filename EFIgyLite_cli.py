...
##If you're running older than OS X 10.10.x then EFI issues are the least of your security concerns
...
class EFIgyCli(object):
    ...
    def __init__(self, api_server_url, quiet=False, debug=False, log_path="", json_results="", batch_json_path="", cacert_path="", only_efi=False):
        ...
        if log_path:
            try:
                self.log_path = os.path.join(os.path.expanduser(os.path.expandvars(log_path)), "%s_%s.log"%(NAME,str(time.time()).replace(".","_")))
        ...
        ##Are we using batch mode where we submit pre-collected values in an json file to send to the API
        self.batch_json_path = batch_json_path
        self.json_batch_template = ["board_id", "smc_ver", "sys_uuid", "build_num", "rom_ver", "hw_ver", "os_ver", "mac_addr"]
        ...
        try:
            self.term_width = int(commands.getoutput("stty size").split(" ")[1])
        except:
            self.term_width = 50
        ...
        if cacert_path:
            self.cacert_path = os.path.expanduser(os.path.expandvars(cacert_path))

        else:
            ##No cacert path specified so try certifi module, if that's not there default to ./cacert.pem that is in the project repo
            try:
                import certifi
        ...
        try:
            ...
            logger.debug("[+] cacert file location: '%s'" % (os.path.abspath(self.cacert_path)))
        except OSError:
    ...
    def _validate_json(self):
        ...
        try:
            ...
            raise
            self.message("[-] Error reading JSON batch file '%s' : '%s'" % (self.batch_json_path, err))
            return False

        ##Does the json represent a dictionary of the expected form?
        if type(batch_json) != types.DictionaryType:
            self.message("[-] JSON batch file '%s' deserialises to unexpected object type '%s'" % (self.batch_json_path, type(batch_json)))
            return False

        ##If it is a dictionary does it have the expected characteristics?
        for endpoint, sys_info in batch_json.items():

            ##Endpoint should be a hostname, IP or some other string identifier, difficult to validate much beyond 'string'
            if type(endpoint) not in [types.StringType, types.UnicodeType]:
                self.message("[-] Element within JSON batch file '%s' conatins unexpected object type for an endpoint element '%s'. %s : %s" % (self.batch_json_path, type(endpoint), endpoint, sys_info))
                return False

            ##Does the sys_info dict contain the expected keys?
            if set(sys_info.keys()).symmetric_difference(set(self.json_batch_template)):
                self.message("[-] Unexpected sys_info structure within JSON batch file %s, expected keys '%s' %s : %s" % (self.batch_json_path, self.json_batch_template, endpoint, sys_info))
                return False


            ##Create a psuedononymised hash of the uuid using MAC addr as salt
            mac_repr = "0x"+sys_info["mac_addr"].lower().replace(":", "")
            sys_info["hashed_uuid"] = hashlib.sha256(mac_repr + sys_info["sys_uuid"]).hexdigest()
        ...
        ##Set the read in json structure as the structure of system data to walk and send to the API
    ...
    def __call__(self):
        ...
        try:
            ...
            ##Not batch, use info from this system to send to API for EFI firmware check
            else:
            ...
            for endpoint, sys_info in self.endpoints_to_check.items():
                ...
                self.message("\tSMC Version      : %s" % (sys_info.get("smc_ver")))
                self.message("\tBoard-ID         : %s" % (sys_info.get("board_id")))
                self.message("\tOS Version       : %s" % (sys_info.get("os_ver")))
    ...
    def gather_system_versions(self):
        ...
        self.endpoints_to_check["127.0.0.1"] = {"hashed_uuid":self.h_sys_uuid, "hw_ver":self.hw_version, "rom_ver":self.efi_version,
    ...
    def check_highest_build(self, sys_info, api_results):
        ...
        if not api_results.get("latest_build_number"):
            self.results[self.current_endpoint]["latest_build_number"] = self.__make_api_get('/apple/latest_build_number/%s' % (".".join(sys_info.get("os_ver").split(".")[:2])))
        ...
        if self._validate_response(api_results["latest_build_number"]):
            ...
            if api_results["latest_build_number"]["msg"] == sys_info.get("build_num"):
                self.message("\t\t[+] SUCCESS - You are running the latest build number (%s) of the OS version you have installed (%s)" % (sys_info.get("build_num"), sys_info.get("os_ver")))

            elif sys_info.get("build_num")[-1].isalpha():
                self.message("\t\t[!] ATTENTION - It looks like you might be running a development OS build '%s' (%s). The EFIgy API currently only has reliable data for production OS releases."%(sys_info.get("build_num"), sys_info.get("os_ver")))
            else:
                self.message("\t\t[-] ATTENTION - You are NOT running the latest release build number of your OS version (%s). Your build number is %s, the latest release build number is %s" % (sys_info.get("os_ver"), sys_info.get("build_num"), api_results["latest_build_number"]["msg"]))
    ...
    def check_os_up_to_date(self, sys_info, api_results):
        ...
        if not api_results.get("latest_os_version"):
            self.results[self.current_endpoint]["latest_os_version"] = self.__make_api_get('/apple/latest_os_version/%s' % (".".join(sys_info.get("os_ver").split(".")[:2])))
        ...
        if self._validate_response(api_results["latest_os_version"]):
            ...
            my_os_ver_num = int("%s%s%s"%(my_os_ver_str[0],my_os_ver_str[1],my_os_ver_str[2]))

            api_os_ver_str = api_results["latest_os_version"]["msg"].split(".")
            api_os_ver_num = int("%s%s%s" % (api_os_ver_str[0], api_os_ver_str[1], api_os_ver_str[2]))

            #if sys_info.get("os_ver") != api_results["latest_os_version"]["msg"]:
            if my_os_ver_num < api_os_ver_num:
                self.message("\t\t[-] ATTENTION - You are NOT running the most up to date version of the OS. Your OS version is %s, the latest versions is %s" % (sys_info.get("os_ver"), api_results["latest_os_version"]["msg"]))

            elif my_os_ver_num > api_os_ver_num:
                self.message("\t\t[!] ATTENTION - It looks like you might be running a development OS build %s. The EFIgy API currently only has reliable data for production OS releases."%(sys_info.get("os_ver")))

            else:
                self.message("\t\t[+] SUCCESS - You are running the latest major/minor/micro version of the OS you have installed (%s)" % (sys_info.get("os_ver")))
    ...
    def check_fw_being_updated(self, sys_info, api_results):
        ...
        if not api_results.get("efi_updates_released"):
            ...
            self.results[self.current_endpoint]["efi_updates_released"] = self.__make_api_get('/apple/no_firmware_updates_released/%s' % (sys_info.get("hw_ver")))
    ...
    def check_fw_versions(self, sys_info, api_results):
        ...
        if not api_results.get("latest_efi_version"):
            ##Call the API to see what the latest version of EFI you are expected to be running given OS ver and mac model
            api_results[self.current_endpoint]["latest_efi_version"] = self.__make_api_get('/apple/latest_efi_firmware/%s/%s' % (sys_info.get("hw_ver"), sys_info.get("build_num")))
        ...
        if self._validate_response(api_results["latest_efi_version"]):
            ...
            if api_efi_str == my_efi_str:
                ...
                ##Looks like you're running a beta or a dev build - pretty much all bets are off here as the dataset doens't cover dev builds but a nicer message makes sense
                self.message("\t\t[!] ATTENTION - It looks like your EFI version (%s) is NEWER than the latest production release that is in the dataset (%s). This is most likely because you are now, or have in the past, installed a developer preview OS and as part of that you also had newer EFI firmware installed. The EFIgy API currently only has reliable data for production OS releases."%(sys_info.get("rom_ver"), api_results["latest_efi_version"]["msg"]))
    ...
    def dump_json(self):
        ...
        if self.json_results == "-":
            ...
            try:
                json_results_fd = open(os.path.expanduser(os.path.expandvars(self.json_results)), "wb")

            except Exception, err:
                self.message("[-] Problem opening file '%s' to write JSON results to: %s"%(self.json_results, err))
                self.message("[!] Defaulting to writing JSON results to stdout instead")
                json_results_fd = sys.stdout

        try:
            json.dump(self.results, json_results_fd)
        except Exception, err:
            self.message("[-] Problem writing JSON output to %s : %s"%(self.json_results, err))

        if self.json_results != "-":
            self.message("[+] Written JSON results to %s" % (os.path.abspath(self.json_results)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-l","--log", help="Directory to log output to, log files within this directory willbe named `EFIgyLite_cli_<$timestamp>.log`")
    parser.add_argument("-j", "--json-output", help="Cause results to be output in JSON format. To write to stdout provide a path of '-', else provide a path to write json results to.")
